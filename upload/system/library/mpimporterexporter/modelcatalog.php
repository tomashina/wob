...
class Modelcatalog extends OpenCartModel {
	...
	public function __construct($registry) {
		...
		// if (!$registry->has('mpimporterexporter_cache_db')) {
		// 	$registry->set('cache_mpimporterexporter', array(
		// 		'name' => 'mpimporterexporter',
		// 		'expires' => 24,
		// 		"time_unit" => 'hours'
		// 	));

		// 	$registry->set('mpimporterexporter_cache_db', new \MpImporterExporter\CacheDB($registry));
		// }

		// $this->db = $registry->get('mpimporterexporter_cache_db');
	}
	...
	public function parseImage($img, $path, $data) {
		...
		if ((utf8_substr($img, 0, 7) === "http://" || utf8_substr($img, 0, 8) === "https://") && $this->isFileContentsExistInHeaders($img) && !$urlofsamewebsite['exists'] )	{
			return $this->saveImgFromURL($img, $path, $data);
	...
	public function saveImgFromURL($url, $path, $data) {
			$img = file_get_contents($url);
			$spath = DIR_IMAGE . $path;

			$this->mkDir(DIR_IMAGE . $path, (!empty($data['dir_permission']) && (int)$data['dir_permission'] ? (int)$data['dir_permission'] : 0777));
	...
	public function isFileContentsExistInHeaders($url, $response_code = 200) {
		...
		if (isset($response[0])) {
			return (utf8_substr($response[0], 9, 3) == $response_code);
		}
