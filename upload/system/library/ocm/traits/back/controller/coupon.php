...
trait Coupon {
    private function getCouponForm($coupon_info, $discount = true, $xcoupon = true) {
        $return = '';
        $return .= $this->ocm->form->get('input', 'name');
        if ($this->getXCouponStatus() && $xcoupon) {
    ...
    private function getXCouponGroups() {
        ...
        if ($this->getXCouponStatus()) {
            ...
            foreach ($method_data as $tab_id => $title) {
                $coupon_groups[$tab_id] = $title;
            }
        }
