...
trait Curl {
    private function curlTab($curl, $placeholders = '', $test = true, $debug = false) {
        ...
        $enc_options = $this->ocm->form->getOptions(array(
            'default' => 'Default',
            'raw' => 'Raw Encoding i.e. spaces turn into %20'
        ), 'none');
        $return .= $this->ocm->form->get('radio', array('name' => 'curl[encoding]', 'title' => 'Encoding Type', 'options' => $enc_options, 'preset' => $curl['encoding'], 'class'=> 'ocm-hide curl_method GET', 'visible' => $curl['method'] == 'GET'));
