...
trait Product {
    ...
    public function getProducts($data = array()) {
        ...
        foreach($rows as &$row) {
            $row['special'] = $this->getProductSpecial($row['product_id']);
            if ($row['special']) {
                $row['text'] = '<i style="text-decoration: line-through;margin-right:5px;">' . number_format($row['price'], 2, '.', '') . '</i>' . number_format($row['special'], 2, '.', '');
            } else {
    ...
    public function getProductSpecial($product_id) {
        $return = false;
        $specials = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' ORDER BY priority, price")->rows;
        foreach ($specials  as $special) {
            if (($special['date_start'] == '0000-00-00' || strtotime($special['date_start']) < time()) && ($special['date_end'] == '0000-00-00' || strtotime($special['date_end']) > time())) {
