...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['heading_title'] = preg_replace('/\s+/u', ' ', trim($product_info['name']));
			...
			if ($product_info['image']) {
				...
				$data['thumb_small'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height'));
			} else {
				$data['thumb'] = '';
				$data['thumb_small'] = '';
			}
			...
			foreach ($results as $result) {
				...
				$data['images'][] = array(
					...
					'large' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_thumb_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_thumb_height')),
			...
			// Težina proizvoda (pretpostavka: već je u kg u $product_info['weight'])
