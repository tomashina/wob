...
class ControllerMpGdprVerificationDataRequest extends Controller {
	...
	public function verification() {
		...
		if(!$json) {
			...
			if(strtotime($today) > strtotime($request_info['expire_on']) && $request_info['status'] == $this->mpgdpr->requestaccess_awating ) {

				// expire the request as timeout
				$this->model_mpgdpr_mpgdpr->updatePersonalDataRequestStatus($request_info['mpgdpr_datarequest_id'], $this->mpgdpr->requestaccess_expire);
		...
		if(!$json) {
			...
			if($request_info['status'] == $this->mpgdpr->requestaccess_awating ) {

				// complete the verification here
				$this->model_mpgdpr_mpgdpr->updatePersonalDataRequestStatus($request_info['mpgdpr_datarequest_id'], $this->mpgdpr->requestaccess_confirmed);
			...
			if($request_info['status'] == $this->mpgdpr->requestaccess_expire ) {
				$json['error'] = $this->language->get('error_code_expire');	
			}
			// if request status is confirmed
			if($request_info['status'] == $this->mpgdpr->requestaccess_confirmed ) {
