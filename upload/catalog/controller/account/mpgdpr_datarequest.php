<?php
class ControllerAccountMpGdprDataRequest extends Controller {
	private $error = array();
	public function index() {
		if ($this->config->get('mpgdpr_login_gdprforms') && !$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/mpgdpr_datarequest', '', $this->mpgdpr->ssl);

			$this->response->redirect($this->url->link('account/login', '', $this->mpgdpr->ssl));
		}

		$this->load->language('mpgdpr/datarequest');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$mpgdpr_datarequest_id = $this->model_mpgdpr_mpgdpr->addPersonalDataRequest($request_data);
			...
			$mpgdpr_requestlist_id = $this->model_mpgdpr_mpgdpr->addRequest($this->mpgdpr->coderequestpersonaldata, $request_data);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('account/mpgdpr_datarequest', '', $this->mpgdpr->ssl)
		);
		...
		$data['text_datarequest'] = $this->language->get('text_datarequest');
		...
		$data['action'] = $this->url->link('account/mpgdpr_datarequest', '', $this->mpgdpr->ssl);
		...
		$this->response->setOutput($this->mpgdpr->view('account/mpgdpr/datarequest', $data));
	}
	...
	private function validate() {
		...
		if(!isset($this->error['email'])) {
			...
			if($customer_id && !isset($this->error['warning'])) {
				...
				$total_requests = $this->model_mpgdpr_mpgdpr->getTodayPersonalDataRequest($customer_id);
