    ...
    $.fn.Tabledit = function(options) {
                    ...
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" value="' + textValue + '" style="display: none;" disabled>';
                                if(settings.columns.editable[i][3] === 'date'){
                                    input = '<input class="tabledit-input tabledit-datepicker ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" value="' + textValue + '" style="display: none;" data-date-format="YYYY-MM-DD">';
                                }
                                else if(settings.columns.editable[i][3] === 'number'){
                                    var step = settings.columns.editable[i][4] ? settings.columns.editable[i][4] : 'any'
                                    var args = settings.columns.editable[i][5] ? settings.columns.editable[i][5] : ''
                                    input = '<input class="tabledit-input ' + settings.inputClass + '" type="number" step="'+ step +'" name="' + settings.columns.editable[i][1] + '" value="' + textValue + '" ' + args + ' style="display: none;" disabled>';
                                }
        ...
        var Mode = {
            view: function(td) {
                ...
                if (settings.editButton) {
                    ...
                    $tr.find('button.tabledit-delete-button').show().html(settings.buttons.delete.html);
                    $tr.find('button.tabledit-edit-button').html(settings.buttons.edit.html).removeClass('active').blur().show();
                }
            },
            edit: function(td) {
                Delete.reset(td);
                ...
                // Update toolbar buttons.
                if (settings.editButton) {
                    $tr.find('button.tabledit-edit-button').html(settings.buttons.cancel.html).addClass('active');
        ...
        var Delete = {
            reset: function(td) {
                ...
                $table.find('.tabledit-edit-button').show();
                // Remove "active" class in delete button.
                $table.find('.tabledit-delete-button').html(settings.buttons.delete.html).removeClass('active').blur();
            },
            submit: function(td) {
                Delete.reset(td);
                ...
                // delete to row.                  
                $(td).parents('table').bootstrapTable('remove', {
                    field: 'id',
                    values: [$(td).parents('tr').find('input.tabledit-identifier').val()]
                })  
            ...
            confirm: function(td) {
                ...
                $(td).find('.tabledit-delete-button').html(settings.buttons.cancel.html).addClass('active');
                // Show confirm button.
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-edit-button', function(event) {
                if (event.handled !== true) {
                    ...
                    $button.parents('tr').find('.input-error').remove()
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                if (event.handled !== true) {
                    ...
                    if(settings.onValidate($(this).parents('tr').find('td.tabledit-edit-mode')) === false) return
