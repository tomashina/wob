   ...
   public function sendOrder()
{
    ...
    private function normalizeSalePayload($sale, string $type = 'order'): array
    {
        ...
        $rootKey = $this->resolveSaleRootKey($sale, $type);
    ...
    private function synchronizeShippingPayload(array $sale, array $order, string $type = 'order'): array
    {
        $shippingNet = $this->getOrderTotalValue($order, 'shipping');
        ...
        $shippingIndex = $this->findShippingItemIndex($sale[$rootKey]['Items'], $order);
    ...
    private function synchronizeNetTotalsPayload(array $sale, array $order, string $type = 'order'): array
    {
        $rootKey = $this->resolveSaleRootKey($sale, $type);
        ...
        foreach ($sale[$rootKey]['Items'] as $item) {
            if ($this->isRoundingItem($item)) {
                continue;
            }
        ...
        if ($targetNetTotal <= 0.0) {
            return $sale;
        }

        $candidateIndexes = [];
        $currentDisplayNetTotal = 0.0;

        foreach ($sale[$rootKey]['Items'] as $index => $item) {
            $quantity = max(1, (int) ($item['quantity'] ?? 1));
            $lineNet = round((float) ($item['netPrice'] ?? 0) * $quantity, 6);

            $currentDisplayNetTotal = round($currentDisplayNetTotal + round($lineNet, 2), 2);
        ...
        $netDiff = round($targetNetTotal - $currentDisplayNetTotal, 2);

        if (abs($netDiff) < 0.01) {
            return $sale;
        }

        $sale[$rootKey]['Items'] = $this->rebalancePayloadNetDiff(
            $sale[$rootKey]['Items'],
            $candidateIndexes,
            $netDiff
        );
    ...
    private function findShippingItemIndex(array $items, array $order): ?int
    {
        $shippingMethod = isset($order['shipping_method']) ? mb_strtolower((string) $order['shipping_method'], 'UTF-8') : '';

        foreach ($items as $index => $item) {
            $values = [
                isset($item['productCode']) ? (string) $item['productCode'] : '',
                isset($item['productName']) ? (string) $item['productName'] : '',
            ...
            foreach ($values as $value) {
                ...
                if (strpos($normalized, 'dostav') !== false || strpos($normalized, 'shipping') !== false || strpos($normalized, 'delivery') !== false) {
                    return $index;
                }

                if ($shippingMethod !== '' && strpos($normalized, $shippingMethod) !== false) {
                    return $index;
                }
            }
        }

        $productCount = isset($order['products']) && is_array($order['products']) ? count($order['products']) : 0;

        if ($productCount > 0 && count($items) > $productCount) {
    ...
    private function resolveSaleRootKey(array $sale, string $type = 'order'): string
    {
        if ($type === 'order' && isset($sale['SalesOrder'])) {
            return 'SalesOrder';
        }

        if ($type !== 'order' && isset($sale['SalesQuote'])) {
            return 'SalesQuote';
        }
    ...
    private function getOrderTotalValue(array $order, string $code): float
    {
        ...
        foreach ($order['totals'] as $total) {
            if (($total['code'] ?? '') === $code) {
                return round((float) ($total['value'] ?? 0), 4);
            }
        }
    ...
    private function getProductsTaxTotal(array $order): float
    {
        if (empty($order['products']) || !is_array($order['products'])) {
            return 0.0;
        }

        $productsTaxTotal = 0.0;

        foreach ($order['products'] as $product) {
            $productsTaxTotal += (float) ($product['tax'] ?? 0) * (int) ($product['quantity'] ?? 0);
        }

        return round($productsTaxTotal, 4);
    }

    private function getShippingTaxTotal(array $order): float
    {
        $shippingTax = $this->getOrderTotalValue($order, 'tax') - $this->getProductsTaxTotal($order);

        return round(max($shippingTax, 0), 4);
    }

    private function getShippingVatRate(array $order): ?float
    {
        $shippingNet = $this->getOrderTotalValue($order, 'shipping');
        ...
        return round(($shippingTax / $shippingNet) * 100, 2);
    }

    private function rebalancePayloadNetDiff(array $items, array $candidateIndexes, float $netDiff): array
    {
        $remainingCents = (int) round($netDiff * 100);

        while ($remainingCents !== 0) {
            $direction = $remainingCents > 0 ? 1 : -1;
            $candidate = $this->findBestPayloadAdjustmentCandidate($items, $candidateIndexes, $direction);

            if ($candidate === null) {
                break;
            }

            $quantity = max(1, (int) ($items[$candidate]['quantity'] ?? 1));
            $lineNet = round((float) ($items[$candidate]['netPrice'] ?? 0) * $quantity, 6);
            $deltaLine = $this->resolveDisplayedNetDelta($lineNet, $direction);

            $items[$candidate]['netPrice'] = $this->formatPayloadDecimal(
                (float) ($items[$candidate]['netPrice'] ?? 0) + ($deltaLine / $quantity),
                6
            );
    ...
    private function findBestPayloadAdjustmentCandidate(array $items, array $candidateIndexes, int $direction): ?int
    {
        $bestIndex = null;
        $bestDelta = null;
        ...
        foreach ($candidateIndexes as $index) {
            ...
            $lineNet = round((float) ($items[$index]['netPrice'] ?? 0) * $quantity, 6);
            $deltaLine = $this->resolveDisplayedNetDelta($lineNet, $direction);

            $unitDelta = abs($deltaLine / $quantity);

            if ($bestDelta === null || $unitDelta < $bestDelta) {
                $bestDelta = $unitDelta;
                $bestIndex = $index;
            }
        }
    ...
    private function resolveDisplayedNetDelta(float $lineNet, int $direction): float
    {
        $displayNet = round($lineNet, 2);

        if ($direction > 0) {
            return round(($displayNet + 0.005) - $lineNet, 6);
        }

        return round(($displayNet - 0.005001) - $lineNet, 6);
    }

    private function isRoundingItem(array $item): bool
    {
        $values = [
            isset($item['productCode']) ? (string) $item['productCode'] : '',
            isset($item['productName']) ? (string) $item['productName'] : '',
        ...
        foreach ($values as $value) {
            ...
            if ($normalized === 'rounding' || strpos($normalized, 'zaokru') !== false) {
                return true;
            }
        }
    ...
    private function isShippingItem(array $item): bool
    {
        $values = [
            isset($item['productCode']) ? (string) $item['productCode'] : '',
            isset($item['productName']) ? (string) $item['productName'] : '',
        ...
        foreach ($values as $value) {
            ...
            if ($normalized === 'dostava' || $normalized === 'dostava ' || $normalized === 'shipping' || $normalized === 'delivery') {
                return true;
            }
    ...
    private function formatPayloadDecimal($value, int $scale = 6): string
    {
