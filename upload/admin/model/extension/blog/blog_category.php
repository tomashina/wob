...
class ModelExtensionBlogBlogCategory extends Model {
	...
	public function getBlogCategories($parent_id, $data = array()) {
		...
		if (!$blog_category_data || !empty($data)) {
			$blog_category_data = array();

			$sql = "SELECT * FROM " . DB_PREFIX . "blog_category c LEFT JOIN " . DB_PREFIX . "blog_category_description cd ON (c.blog_category_id = cd.blog_category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'";

            if(!empty($data['filter_name'])) {
                $sql .= " AND LOWER(cd.name) LIKE LOWER('%" . $this->db->escape($data['filter_name']) . "%')" ;
            }
			...
			foreach ($query->rows as $result) {
				...
				$blog_category_data = array_merge($blog_category_data, $this->getBlogCategories($result['blog_category_id']), $data);
			}	
