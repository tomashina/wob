...
class ModelExtensionMpImporterExporterImportProduct extends \MpImporterExporter\Model {
	...
	public function addProduct($data, $find_data) {
		...
		if (!empty($data['attribute']) && !empty($find_data['cell_operations']['store'])) {
			foreach ($data['attribute'] as $product_attribute) {
				$product_attribute = array_map('trim', explode('::', $product_attribute));
		...
		if (!empty($data['discount']) && !empty($find_data['cell_operations']['discount'])) {
			foreach ($data['discount'] as $product_discount) {
				$product_discount = array_map('trim', explode('::', $product_discount));
		...
		if (!empty($data['special']) && !empty($find_data['cell_operations']['special'])) {
			foreach ($data['special'] as $product_special) {
				$product_special = array_map('trim', explode('::', $product_special));
		...
		if(!empty($find_data['cell_operations']['category_ids'])) {
			...
			if (!empty($data['category_ids'])) {
				...
				foreach ($data['category_name'] as $category_name_value) {
					$category_name = array_map('trim', explode('&gt;', $category_name_value));
					...
					if($category_info) {
						...
						$inserted_category_names = array_map('trim', explode('&gt;', $category_name_value));
						$parent_id = 0;
						$number_row = 1;
		...
		if (!empty($data['filter']) && !empty($find_data['cell_operations']['filter'])) {
			foreach ($data['filter'] as $filter) {
				$filter = trim($filter);
				if($filter) {
					$filter_data = array_map('trim', explode('-', $filter));
		...
		if (!empty($data['options']) && !empty($find_data['cell_operations']['options'])) {
			foreach ($data['options'] as $product_option) {
				$product_option = array_map('trim', explode('::', $product_option));
					...
					if($option_info) {
						if ($product_option[1] == 'select' || $product_option[1] == 'radio' || $product_option[1] == 'checkbox' || $product_option[1] == 'image') {
							if (isset($product_option[3])) {
								...
								$product_option_values = array_map('trim', explode('||', $product_option[3]));
								
								foreach ($product_option_values as $product_option_value) {
									$product_option_value = array_map('trim', explode('^^', $product_option_value));
		...
		if(!empty($find_data['review'])) {
			if (!empty($data['reviews'])) {
				foreach ($data['reviews'] as $review) { 
					$review = array_map('trim', explode('::', $review));
					if (count($review) >= 5) {
...
if (VERSION <= '2.2.0.0') {
	class ModelMpImporterExporterImportProduct extends ModelExtensionMpImporterExporterImportProduct { }
}