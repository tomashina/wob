	...
	public function index() {
		...
		if ($product_info) {
			...
			$this->document->addStyle('catalog/view/javascript/jquery/datetimepicker/bootstrap-datetimepicker.min.css');
				...
				if (isset($h1) && strlen($h1) > 5){
					$data['heading_title'] = $h1;
				}else{
					$data['heading_title'] = $product_info['name'];
				}
				
				$data['h2'] = (isset($h2) && strlen($h2) > 5)? $h2 : '';
				$data['image_alt'] = (isset($image_alt) && strlen($image_alt)>5)? $image_alt : $product_info['name']; 
				$data['image_title'] = (isset($image_title) && strlen($image_title)>5)? $image_title : $product_info['name']; 
