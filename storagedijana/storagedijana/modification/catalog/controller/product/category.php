	...
	public function index() {
		...
		if ($category_info) {
			...
			$this->document->setKeywords($category_info['meta_keyword']);
				...
				$data['heading_title'] = (isset($h1) && strlen($h1) > 5) ? $h1 : $category_info['name'];				
				$data['h2'] = (isset($h2) && strlen($h2)>5)? $h2 : '';
				$data['image_alt'] = (isset($image_alt) && strlen($image_alt) > 5)? $image_alt : $category_info['name'];
				$data['image_title'] = (isset($image_title) && strlen($image_title) > 5)? $image_title : $category_info['name'];
