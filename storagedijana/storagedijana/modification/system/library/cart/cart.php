...
class Cart {
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($this->config->get('module_boost_special_manager_include') == "1") {
				$queryx = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$cart['product_id'] . "'");
				...
				foreach ($categories  as $category) {
					if ($category['discount_type'] == "0") {
						$new_price = ($price_special - (($category['discount']/100) * $price_special));
						$price = round($new_price,2);
					} elseif ($category['discount_type'] == "1") {
						$price = ($price_special - $category['discount']);
					} else {
